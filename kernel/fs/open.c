...
*/	case O_RDWR | O_APPEND:
	...
	dbg(DBG_PRINT,"(GRADING2B)\n");
	vnode_t *res_node;
	int retval = open_namev(filename, oflags, &res_node, NULL);
	if(retval < 0){
		dbg(DBG_PRINT,"(GRADING2B)\n");
		fput(file);
		return retval;
	}
/*	if(((res_node->vn_cdev == NULL) && S_ISCHR(res_node->vn_mode))
	    		||((res_node->vn_bdev == NULL) && S_ISBLK(res_node->vn_mode))){
		dbg(DBG_PRINT,"(GRADING2) newpath\n");
    	fput(file);
    	vput(res_node);
        return -ENXIO;
    }
*/	if(S_ISDIR(res_node->vn_mode) && (file->f_mode & FMODE_WRITE)){
		dbg(DBG_PRINT,"(GRADING2B)\n");
		fput(file);
		vput(res_node);
		return -EISDIR;
