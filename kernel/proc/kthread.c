...
kthread_create(struct proc *p, kthread_func_t func, long arg1, void *arg2)
{
	...
	kthread_t *thr = slab_obj_alloc(kthread_allocator);/*kmalloc(sizeof(kthread_t));*/
    memset(thr, 0, sizeof(kthread_t));

    thr->kt_kstack = (char*)alloc_stack();
    thr->kt_retval = NULL;
    thr->kt_errno = 0;
    thr->kt_proc = p;
    thr->kt_cancelled = 0;
    ...
    /*
    ktqueue_enqueue(&thr->kt_wchan, thr->kt_qlink);
	*/

    list_insert_tail(&(thr->kt_proc->p_threads), &(thr->kt_plink));
/*
#ifdef __MTP__
    thr->kt_detached = 0;
    thr->kt_joinq = NULL;
...
*/
    ...
    context_setup(&thr->kt_ctx, func, arg1, arg2,
    		thr->kt_kstack, DEFAULT_STACK_SIZE, thr->kt_proc->p_pagedir);
    /*
    dbg(DBG_THR, "thread created for process %s ,pid = %d\n", thr->kt_proc->p_comm, thr->kt_proc->p_pid);
    */
...
kthread_cancel(kthread_t *kthr, void *retval)
{
	...
	if(kthr == curthr){
		...
		kthr->kt_cancelled = 1;/*TODO: set it here?*/
		sched_cancel(kthr);
		dbg(DBG_PRINT, "(GRADING1C)\n");
