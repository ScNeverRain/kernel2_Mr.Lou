...
int sched_cancellable_sleep_on(ktqueue_t *q) {
	/* TODO: kernel assignment 01 sched_cancellable_sleep_on*/
	...
	ktqueue_enqueue(q, curthr);
	KASSERT(&kt_runq != curthr->kt_wchan);
	sched_switch();
	dbg(DBG_PRINT, "(GRADING1C 5)\n");
	if (curthr->kt_cancelled == 1) {
		dbg(DBG_PRINT, "(GRADING1C 5)\n");
...
sched_wakeup_on(ktqueue_t *q) {
	...
	if (thr != NULL) {
		KASSERT(
				(thr->kt_state == KT_SLEEP) || (thr->kt_state == KT_SLEEP_CANCELLABLE));
...
void sched_switch(void) {
	...
	intr_setipl(IPL_HIGH);         /* protect access to RunQueue by */
